  ...
  try {
    ...
    if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
      ...
      if (hasToolUse) {
        ...
        sendToRenderer('agentStatus', { id: agentId, status: 'active' })
        ...
        if (hasNonExemptTool) {
          startPermissionTimer(agentId, agents, permissionTimers, PERMISSION_EXEMPT_TOOLS, sendToRenderer)
        }
        ...
        startWaitingTimer(agentId, 5000, agents, waitingTimers, sendToRenderer)
      }
    } else if (record.type === 'progress') {
      processProgressRecord(agentId, record, agents, waitingTimers, permissionTimers, sendToRenderer)
      ...
      if (Array.isArray(content)) {
        ...
        if (hasToolResult) {
          ...
          cancelWaitingTimer(agentId, waitingTimers)
          clearAgentActivity(agent, agentId, permissionTimers, sendToRenderer)
          agent.hadToolsInTurn = false
        }
      ...
      if (agent.activeToolIds.size > 0) {
        ...
        sendToRenderer('agentToolsClear', { id: agentId })
      }
  ...
  if (msgType === 'assistant') {
    ...
    for (const block of content) {
      if (block.type === 'tool_use' && block.id) {
        ...
        sendToRenderer('subagentToolStart', {
          id: agentId,
          parentToolId,
          toolId: block.id,
          status,
    ...
    for (const block of content) {
      if (block.type === 'tool_result' && block.tool_use_id) {
        ...
        setTimeout(() => {
          sendToRenderer('subagentToolDone', {
            id: agentId,
            parentToolId,
            toolId,
          })
