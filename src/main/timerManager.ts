...
export function startWaitingTimer(
  ...
  const timer = setTimeout(() => {
    ...
    sendToRenderer('agentStatus', { id: agentId, status: 'waiting' })
  }, delayMs)
...
export function startPermissionTimer(
  ...
  const timer = setTimeout(() => {
    ...
    if (hasNonExempt) {
      ...
      sendToRenderer('agentToolPermission', { id: agentId })
      // Also notify stuck sub-agents
      for (const parentToolId of stuckSubagentParentToolIds) {
        sendToRenderer('subagentToolPermission', { id: agentId, parentToolId })
      }
    }
  }, 7000) // PERMISSION_TIMER_DELAY_MS
